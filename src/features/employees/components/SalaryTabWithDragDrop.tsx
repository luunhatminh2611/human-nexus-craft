...
function DraggableSalaryItem({ item }: { item: SalaryItem }) {
  ...
  return (
    <div
      ref={drag}
      className={`p-3 border rounded-lg hover:bg-accent/50 transition-colors cursor-move ${isDragging ? 'opacity-50' : ''
        }`}
    >
...
function CustomSalaryDropZone({
  ...
  return (
    <div
      ref={drop}
      className={`min-h-[200px] border-2 border-dashed rounded-lg p-4 transition-colors ${isOver ? 'border-primary bg-primary/5' : 'border-muted'
        }`}
    >
      {items.length === 0 ? (
...
export function SalaryTabWithDragDrop({ employee }: { employee: Employee | null }) {
  ...
  const currentEmployee = employee && employee.salary ? employee : defaultEmployee;
  const [customItems, setCustomItems] = useState<SalaryItem[]>(
    currentEmployee.customSalaryItems || []
  );
  ...
  }, [currentEmployee.salary.base, getDefaultStructureItems, customItems]);
