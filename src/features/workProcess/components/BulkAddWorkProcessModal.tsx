...
import {
  Loader2, X, Plus, Copy, ChevronDown, Search, User, Briefcase,
} from 'lucide-react';
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function BulkAddWorkProcessModal({ isOpen, onClose, onSuccess, mode, initialRows }: Props) {
  ...
  const copyRow = (id: number) => {
    ...
    if (row) {
      setRows(prev => [
        ...prev,
        { ...row, tempId: idCounter.current++, rowStatus: 'pending', errorMessage: undefined },
      ]);
  ...
  const handleSubmit = async () => {
    ...
    const grouped = rows.reduce<Record<string, WorkProcessRow[]>>((acc, row) => {
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {rows.length === 0 ? (
              <div className="text-sm text-muted-foreground text-center py-16 flex flex-col items-center gap-3">
                ...
                <p>Chưa có dòng nào. Nhấn <b>Thêm dòng</b> để bắt đầu.</p>
              </div>
            ) : (
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="whitespace-nowrap min-w-[160px]">Ngày bắt đầu *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[160px]">Ngày kết thúc *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[280px]">Nội dung công tác *</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {rows.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <Input
                            value={row.detail}
                            onChange={e => update(row.tempId, 'detail', e.target.value)}
                            className="h-8 text-sm"
