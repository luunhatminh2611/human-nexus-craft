// components/WorkProcessEditModal.tsx
...
export default function WorkProcessEditModal({
  ...
  const update = (id: number, field: keyof WorkProcess, value: string) =>
        ...
        {activeRows.length === 0 ? (
          ...
          <div className="space-y-3">
            {activeRows.map((item, index) => (
              <div key={item.id} className={`p-4 border rounded-lg space-y-3 ${item.isNew ? 'border-blue-300 bg-blue-50/50' : ''}`}>
                {/* Header */}
