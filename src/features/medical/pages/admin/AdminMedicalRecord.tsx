...
import {
  ...
  AlertCircle, Activity, Heart, User,
...
function StatsBar({
  ...
  const scoped =
    filterMonth === 'all'
      ? records
      : records.filter(r => r.ngayKham?.slice(0, 7) === filterMonth);
  ...
  const stats = [
    ...
    {
      label: filterMonth === 'all' ? 'Tất cả' : 'Trong tháng',
      value: scoped.length,
      icon: CalendarDays,
      color: 'text-blue-600',
      bg: 'bg-blue-50',
    ...
    {
      label: 'Sức khỏe tốt (I–II)',
      value: scoped.filter(r => r.plSucKhoe && r.plSucKhoe <= 2).length,
      icon: Heart,
      color: 'text-emerald-600',
      bg: 'bg-emerald-50',
    },
    {
      label: 'Cần theo dõi (III+)',
      value: scoped.filter(r => r.plSucKhoe && r.plSucKhoe >= 3).length,
      icon: Activity,
