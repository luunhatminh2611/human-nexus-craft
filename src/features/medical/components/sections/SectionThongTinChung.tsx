...
export default function SectionThongTinChung({
  form,
  set,
  errors,
  num,
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <Field label="Năm sinh">
          <Input type="date" value={form.birthday ?? ""} disabled />
        </Field>
        {/* [3] Chức danh */}
        <Field label="Chức danh">
          <Input value={form.jobTitleName ?? ""} disabled />
        </Field>
        ...
        {/* [5] Đơn vị */}
        <Field label="Tập đoàn/Công ty">
          <Input value={form.donVi ?? ""} disabled />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <Field label="Mạch (lần/phút)">
          <Input
            type="number"
            value={form.mach ?? ""}
            onChange={(e) => set("mach", num(e.target.value))}
          />
        </Field>
        {/* [10] Huyết áp */}
        <Field label="Huyết áp (mmHg)">
          <Input
            value={form.huyetAp ?? ""}
            onChange={(e) => set("huyetAp", e.target.value)}
            placeholder="120/80"
          />
        </Field>
        {/* [11] PL Thể lực */}
        <Field label="PL Thể lực">
          <Select
            value={form.plTheLuc?.toString() ?? ""}
            onValueChange={(v) => set("plTheLuc", Number(v))}
          >
            <SelectTrigger>
        ...
        <Field label="PL Sức khỏe">
          <Select
            value={form.plSucKhoe?.toString() ?? ""}
            onValueChange={(v) => set("plSucKhoe", Number(v))}
          >
            <SelectTrigger>
              <SelectValue placeholder="Chọn loại" />
            </SelectTrigger>
        ...
        <Field label="Bệnh thông thường">
          <Input
            value={form.benhThongThuong ?? ""}
            onChange={(e) => set("benhThongThuong", e.target.value)}
          />
        </Field>
        {/* [14] Mã bệnh thông thường */}
        <Field label="Mã bệnh thông thường (ICD-10)">
          <Input
            value={form.maBenhThongThuong ?? ""}
            onChange={(e) => set("maBenhThongThuong", e.target.value)}
            placeholder="ICD-10"
          />
        </Field>
        {/* [15] Số ngày nghỉ ốm */}
        <Field label="Số ngày nghỉ ốm">
          <Input
            type="number"
            value={form.soNgayNghiOm ?? ""}
            onChange={(e) => set("soNgayNghiOm", num(e.target.value))}
          />
        </Field>
        {/* [16] Bệnh mãn tính */}
        <Field label="Bệnh mãn tính">
          <Input
            value={form.benhManTinh ?? ""}
            onChange={(e) => set("benhManTinh", e.target.value)}
          />
        </Field>
        {/* [17] Mã bệnh mãn tính */}
        <Field label="Mã bệnh mãn tính (ICD-10)">
          <Input
            value={form.maBenhManTinh ?? ""}
            onChange={(e) => set("maBenhManTinh", e.target.value)}
            placeholder="ICD-10"
          />
        </Field>
        ...
        <Field label="Phân loại nghề nghiệp">
          <Select
            value={form.phanLoaiNgheNghiep?.toString() ?? ""}
            onValueChange={(v) => set("phanLoaiNgheNghiep", Number(v))}
          >
            <SelectTrigger>
              <SelectValue placeholder="Chọn loại" />
            </SelectTrigger>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-6 mb-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <Checkbox
              checked={Boolean(form.nldTiepXucYeuToCoHai)}
              onCheckedChange={(v) => set("nldTiepXucYeuToCoHai", v)}
            />
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <Checkbox
              checked={Boolean(form.biTaiNanLaoDong)}
              onCheckedChange={(v) => set("biTaiNanLaoDong", v)}
            />
        ...
        {form.biTaiNanLaoDong && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-3 bg-muted/40 rounded-lg">
            ...
            <Field label="[21] Ngày bị TNLĐ">
              <Input
                type="date"
                value={form.ngayBiTaiNanLaoDong ?? ""}
                onChange={(e) => set("ngayBiTaiNanLaoDong", e.target.value)}
              />
            </Field>
            ...
            <Field label="[22] Số ngày điều trị TNLĐ">
              <Input
                type="number"
                value={form.soNgayDieuTriTnld ?? ""}
                onChange={(e) => set("soNgayDieuTriTnld", num(e.target.value))}
              />
            </Field>
            ...
            <Field label="[23] Tỷ lệ giám định TNLĐ (%)">
              <Input
                type="number"
                value={form.tyLeGiamDinhTnld ?? ""}
                onChange={(e) => set("tyLeGiamDinhTnld", num(e.target.value))}
              />
            </Field>
            ...
            <Field label="[24] Năm hưởng trợ cấp TNLĐ">
              <Input
                type="number"
                value={form.namHuongTroCapTnld ?? ""}
                onChange={(e) => set("namHuongTroCapTnld", num(e.target.value))}
              />
            </Field>
          </div>
        )}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-6 mb-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <Checkbox
              checked={Boolean(form.nldKskPhatHienBnn)}
              onCheckedChange={(v) => set("nldKskPhatHienBnn", v)}
            />
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <Checkbox
              checked={Boolean(form.nldChanDoanBnn)}
              onCheckedChange={(v) => set("nldChanDoanBnn", v)}
            />
        ...
        {(form.nldKskPhatHienBnn || form.nldChanDoanBnn) && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-3 bg-muted/40 rounded-lg">
            <Field label="[27] Chức danh nghề khi mắc BNN">
              <Input
                value={form.chucDanhNgheKhiMacBnn ?? ""}
                onChange={(e) => set("chucDanhNgheKhiMacBnn", e.target.value)}
              />
            </Field>
            <Field label="[28] Tên bệnh nghề nghiệp">
              <Input
                value={form.tenBenhNgheNghiep ?? ""}
                onChange={(e) => set("tenBenhNgheNghiep", e.target.value)}
              />
            </Field>
            <Field label="[29] Mã bệnh nghề nghiệp">
              <Input
                value={form.maBenhNgheNghiep ?? ""}
                onChange={(e) => set("maBenhNgheNghiep", e.target.value)}
              />
            </Field>
            ...
            {form.nldChanDoanBnn && (
              <>
                <Field label="[30] Thời gian hội chẩn BNN">
                  <Input
                    type="date"
                    value={form.thoiGianHoiChanBnn ?? ""}
                    onChange={(e) => set("thoiGianHoiChanBnn", e.target.value)}
                  />
                </Field>
                <Field label="[31] Thể bệnh">
                  <Input
                    value={form.theBenh ?? ""}
                    onChange={(e) => set("theBenh", e.target.value)}
                  />
                </Field>
                <Field label="[32] Năm giám định BNN">
                  <Input
                    type="number"
                    value={form.namGiamDinhBnn ?? ""}
                    onChange={(e) => set("namGiamDinhBnn", num(e.target.value))}
                  />
                </Field>
                <Field label="[33] Tỷ lệ giám định BNN (%)">
                  <Input
                    type="number"
                    value={form.tyLeGiamDinhBnn ?? ""}
                    onChange={(e) =>
                      set("tyLeGiamDinhBnn", num(e.target.value))
                    }
                  />
                </Field>
                <Field label="[34] Năm hưởng trợ cấp BNN">
                  <Input
                    type="number"
                    value={form.namHuongTroCapBnn ?? ""}
                    onChange={(e) =>
                      set("namHuongTroCapBnn", num(e.target.value))
                    }
                  />
                </Field>
              </>
        ...
        <div className="flex flex-wrap gap-6 mt-3 mb-2">
          <label className="flex items-center gap-2 cursor-pointer">
            <Checkbox
              checked={Boolean(form.daRuaPhoi)}
              onCheckedChange={(v) => set("daRuaPhoi", v)}
            />
        ...
        {form.daRuaPhoi && (
          <Field label="[36] Năm rửa phổi">
            <Input
              type="number"
              value={form.namRuaPhoi ?? ""}
              onChange={(e) => set("namRuaPhoi", num(e.target.value))}
            />
          </Field>
        )}
        ...
        {(form.nldKskPhatHienBnn || form.nldChanDoanBnn) && !form.daRuaPhoi && (
          <Field label="[37] NLĐ chống chỉ định rửa phổi">
            <Input
              value={form.chongChiDinhRuaPhoi ?? ""}
              onChange={(e) => set("chongChiDinhRuaPhoi", e.target.value)}
            />
          </Field>
        )}
        {/* [38] Tiền sử bệnh, tật của gia đình */}
        <Field
          label="[38] Tiền sử bệnh. tật của gia đình"
          className="md:col-span-3"
        >
          <Input
            value={form.tienSuBenhGiaDinh ?? ""}
            onChange={(e) => set("tienSuBenhGiaDinh", e.target.value)}
          />
        </Field>
      </div>
    </div>
