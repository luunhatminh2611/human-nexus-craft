// components/BulkAddSocialInsuranceModal.tsx
...
import {
  ...
  ChevronDown, Search, User, Paperclip, ShieldCheck,
} from 'lucide-react';
    ...
    <div className="flex items-center gap-1.5 min-w-[150px]">
      ...
      {row.file ? (
        <div className="flex items-center gap-1 flex-1 min-w-0">
          ...
          <span
            className="text-xs text-green-700 truncate flex-1 max-w-[100px]"
            title={row.file.name}
          >
            {row.file.name}
          </span>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function BulkAddSocialInsuranceModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const copyRow = (id: number) => {
    ...
    if (row) {
      setRows(prev => [
        ...prev,
        { ...row, tempId: idCounter.current++, file: null, rowStatus: 'pending', errorMessage: undefined },
      ]);
      toast.info('Đã sao chép dòng (cần chọn lại file)');
    }
  };

  const update = (id: number, field: keyof InsuranceRow, value: any) =>
  ...
  const handleExportTemplate = async () => {
    try {
      ...
      ws.columns = [
        ...
        { header: 'Số sổ BHXH *',                               key: 'insuranceBookNumber',  width: 22 },
        { header: 'Mã số BHXH *',                               key: 'insuranceCode',        width: 20 },
        { header: 'Ngày tham gia * (YYYY-MM-DD)',                key: 'startDate',            width: 28 },
        { header: 'Trạng thái * (ACTIVE/INACTIVE/SUSPENDED)',   key: 'status',               width: 42 },
        { header: 'Mức lương đóng (VNĐ) *',                    key: 'salaryBase',           width: 26 },
      ...
      // Sheet tham khảo trạng thái
      const wsRef = wb.addWorksheet('Trạng thái');
      wsRef.columns = [
        { header: 'Giá trị', key: 'value', width: 18 },
        { header: 'Mô tả',   key: 'label', width: 24 },
      ];
      wsRef.getRow(1).font = { bold: true };
      STATUS_OPTIONS.forEach(s => wsRef.addRow({ value: s.value, label: s.label }));
  ...
  const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const ws = wb.getWorksheet('BHXH');
      if (!ws) throw new Error('Không tìm thấy sheet "BHXH"');

      const imported: InsuranceRow[] = [];
      ws.eachRow((row, n) => {
        if (n === 1) return;
        const g = (c: number) => {
      ...
      toast.error(err.message || 'Không thể đọc file Excel');
    } finally {
      e.target.value = '';
    }
  };
  ...
  const validate = () => {
    ...
    setRows(prev => prev.map(r => {
      ...
      if (!r.insuranceBookNumber.trim())    errs.push('Thiếu số sổ BHXH');
      if (!r.insuranceCode.trim())          errs.push('Thiếu mã số BHXH');
      if (!r.startDate)                     errs.push('Thiếu ngày tham gia');
      if (!r.status)                        errs.push('Chưa chọn trạng thái');
      if (!r.salaryBase || Number(r.salaryBase) <= 0) errs.push('Mức lương không hợp lệ');
  ...
  const handleSubmit = async () => {
    ...
    for (const row of rows) {
      try {
        const payload = {
          ...
          insuranceBookNumber: row.insuranceBookNumber,
          insuranceCode:       row.insuranceCode,
          startDate:           row.startDate,
          status:              row.status,
          salaryBase:          Number(row.salaryBase),
          note:                row.note || null,
        };
        await employeeSocialInsuranceApi.create(payload, row.file!);
        setRows(prev => prev.map(r =>
          r.tempId === row.tempId ? { ...r, rowStatus: 'success' } : r,
        ));
        ok++;
      } catch (err: any) {
        setRows(prev => prev.map(r =>
          r.tempId === row.tempId
            ? { ...r, rowStatus: 'error', errorMessage: err?.response?.data?.message || 'Lỗi khi lưu' }
            : r,
        ));
        fail++;
      }
    ...
    if (!fail) {
      toast.success(`Đã thêm ${ok} bản ghi BHXH thành công`);
      onSuccess?.();
      handleClose();
    } else {
      toast.warning(`${ok} thành công · ${fail} thất bại — kiểm tra dòng đỏ`);
    }
  };
      ...
      <DialogContent className="max-w-[96vw] w-full h-[92vh] flex flex-col p-0">
        <DialogHeader className="px-6 py-4 border-b shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            Thêm bảo hiểm xã hội hàng loạt
          </DialogTitle>
          <DialogDescription>
            Thêm thông tin BHXH cho nhiều nhân viên cùng lúc
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden flex flex-col min-h-0">
          ...
          <div className="px-6 pt-3 shrink-0">
            <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded px-3 py-2">
              ⚠️ File tài liệu phải được chọn thủ công cho từng dòng. Nhập Excel chỉ điền thông tin còn lại.
            </p>
          </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="whitespace-nowrap min-w-[170px]">Nhân viên *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[160px]">Số sổ BHXH *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[150px]">Mã số BHXH *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[150px]">Ngày tham gia *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[160px]">Trạng thái *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[150px]">Mức lương đóng (VNĐ) *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[160px]">File *</TableHead>
                      <TableHead className="whitespace-nowrap min-w-[160px]">Ghi chú</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {rows.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell className="sticky left-10 bg-inherit z-10 w-20 border-r">
                          <div className="flex gap-0.5">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => removeRow(row.tempId)} title="Xóa dòng">
                        ...
                        <TableCell>
                          <EmployeePopover
                            row={row} allEmployees={allEmployees}
                            onSelect={emp => {
                        ...
                        <TableCell>
                          <Input
                            value={row.insuranceBookNumber}
                            onChange={e => update(row.tempId, 'insuranceBookNumber', e.target.value)}
                        ...
                        <TableCell>
                          <Input
                            value={row.insuranceCode}
                            onChange={e => update(row.tempId, 'insuranceCode', e.target.value)}
                            className="h-8 text-sm"
                            placeholder="0100100001"
                          />
                        </TableCell>
                        {/* Ngày tham gia */}
                        <TableCell>
                        ...
                        <TableCell>
                          <Select
                            value={row.status}
                            onValueChange={v => update(row.tempId, 'status', v)}
                          >
                        ...
                        <TableCell>
                          <Input
                            ...
                            onChange={e => update(row.tempId, 'salaryBase', e.target.value)}
                            className="h-8 text-sm"
                            placeholder="6000000"
                          />
                        </TableCell>
                        ...
                        <TableCell>
                          <Input
                            value={row.note}
                            onChange={e => update(row.tempId, 'note', e.target.value)}
                            className="h-8 text-sm"
                            placeholder="Ghi chú..."
                          />
                        </TableCell>
