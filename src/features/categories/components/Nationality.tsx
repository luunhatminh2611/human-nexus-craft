...
export default function NationalityTab() {
  ...
  const [selectedItem, setSelectedItem] = useState<Nationality | null>(null);
  ...
  const fetchNationalities = async () => {
    try {
      ...
      toast.error('Không thể tải danh sách quốc tịch');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const paginatedNationalities = useMemo(() => {
    ...
    return filteredNationalities.slice(startIndex, startIndex + itemsPerPage);
  }, [filteredNationalities, currentPage, itemsPerPage]);
  ...
  const handleSubmit = async () => {
    if (!formData.name) {
      toast.error('Vui lòng nhập tên quốc tịch');
      return;
    }
    ...
    try {
      setLoading(true);
      if (selectedItem) {
        await nationalityApi.update(selectedItem.id, formData);
        toast.success('Cập nhật quốc tịch thành công');
      } else {
        await nationalityApi.create(formData);
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await nationalityApi.delete(selectedItem.id);
      toast.success('Xóa quốc tịch thành công');
      setIsDeleteDialogOpen(false);
      setSelectedItem(null);
      fetchNationalities();
    } catch (error) {
      toast.error('Xóa quốc tịch thất bại');
      console.error(error);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-4 p-4">
      <div className="grid grid-cols-6 items-center gap-4">
        <div className="flex col-span-6 items-center gap-2">
          ...
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Tìm kiếm theo mã, tên quốc tịch..."
              value={searchTerm}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {selectedItem ? 'Cập nhật quốc tịch' : 'Thêm quốc tịch mới'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="code">Mã quốc tịch</Label>
              <Input
                id="code"
                value={formData.code}
                onChange={(e) =>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <p>
            Bạn có chắc chắn muốn xóa quốc tịch <strong>"{selectedItem?.name}"</strong> không?
          </p>
          <DialogFooter>
            <Button
              variant="outline"
