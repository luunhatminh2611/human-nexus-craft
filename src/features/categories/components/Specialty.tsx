...
export default function SpecialtyTab() {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) {
      toast({ title: 'Lỗi', description: 'Vui lòng nhập tên chuyên môn', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      if (editing) {
        ...
        toast({
          title: 'Thành công',
          description: 'Thêm mới chuyên môn thành công',
        });
      }
      ...
      toast({
        title: 'Lỗi',
        description: `Không thể ${editing ? 'cập nhật' : 'thêm mới'} chuyên môn`,
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast({ title: 'Thành công', description: 'Xóa chuyên môn thành công' });
      loadSpecialties(); // Reload lại danh sách
    } catch (error) {
      toast({
        title: 'Lỗi',
        description: 'Không thể xóa chuyên môn',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? 'Chỉnh sửa' : 'Thêm mới'} chuyên môn</DialogTitle>
