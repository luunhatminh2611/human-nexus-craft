...
import BulkAddTrainingMajorModal from '../components/modal/BulkAddTrainingMajorModal';
import BulkEditTrainingMajorModal from '../components/modal/BulkEditTrainingMajorModal';
...
export default function TrainingMajorTab() {
  ...
  // Lọc và phân trang
  const filteredTrainingMajors = useMemo(() => {
    return trainingMajors.filter(item => {
  ...
  }, [trainingMajors, searchTerm]);

  const totalPages = Math.ceil(filteredTrainingMajors.length / itemsPerPage);

  const paginatedTrainingMajors = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return filteredTrainingMajors.slice(startIndex, startIndex + itemsPerPage);
  }, [filteredTrainingMajors, currentPage, itemsPerPage]);
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    if (checked) {
      setSelectedIds(paginatedTrainingMajors.map(item => item.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  useEffect(() => {
    if (selectedIds.length === paginatedTrainingMajors.length && paginatedTrainingMajors.length > 0) {
      setSelectAll(true);
    } else {
      setSelectAll(false);
    }
  }, [selectedIds, paginatedTrainingMajors]);
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="grid grid-cols-6 items-center gap-4">
        <div className="flex col-span-6 items-center gap-2">
          ...
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Tìm kiếm theo mã, tên ngành đào tạo..."
              value={searchTerm}
        ...
        <Table>
          <TableHeader className='bg-muted'>
            <TableRow>
              ...
              <TableHead className="border">Mã ngành</TableHead>
              <TableHead className="border">Tên ngành đào tạo</TableHead>
          ...
          <TableBody>
            {loading ? (
            ...
            ) : paginatedTrainingMajors.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-center">
              ...
              paginatedTrainingMajors.map((item, index) => (
      ...
      {/* Pagination */}
      {filteredTrainingMajors.length > 0 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-500">
            Hiển thị {(currentPage - 1) * itemsPerPage + 1} -{' '}
            {Math.min(currentPage * itemsPerPage, filteredTrainingMajors.length)} trong tổng số{' '}
            {filteredTrainingMajors.length} ngành đào tạo
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {selectedItem ? 'Cập nhật ngành đào tạo' : 'Thêm ngành đào tạo mới'}
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <p>
            Bạn có chắc chắn muốn xóa ngành đào tạo <strong>"{selectedItem?.name}"</strong> không?
          </p>
          <DialogFooter>
            <Button
              variant="outline"
