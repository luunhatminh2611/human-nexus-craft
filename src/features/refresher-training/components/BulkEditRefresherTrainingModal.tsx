...
export default function BulkEditRefresherTrainingModal({
  isOpen,
  onClose,
  onSuccess,
}: Props) {
  ...
  useEffect(() => {
    ...
    setRows((prev) => (prev.length ? prev : [makeRow()]));
  }, [isOpen]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(`Đã thêm ${rows.length} dòng thành công`);
                ...
                <TableBody>
                  {rows.map((row, index) => (
                    ...
                    >
                      ...
                      <TableCell className="sticky left-10 bg-inherit z-10 border-r">
