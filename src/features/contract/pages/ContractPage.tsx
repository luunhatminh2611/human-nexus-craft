...
import { Search, Eye, ChevronLeft, ChevronRight, Plus, Edit, FileText, Trash2, Download, Upload, ListPlus } from 'lucide-react';
...
import BulkAddContractModal from '../components/BulkAddContractModal';
...
export default function ContractPage() {
  ...
  const handleExport = async () => {
    try {
      ...
      toast({ title: 'Lỗi', description: 'Không thể tải xuống file', variant: 'destructive' });
    }
  };
      ...
      <div className="flex items-center justify-between">
        {isAdmin ? (
          ...
          <div>
            ...
            <p className="text-muted-foreground">Xem danh sách hợp đồng lao động của bạn</p>
          </div>
        )}
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'active' | 'inactive')}>
        <TabsList>
          <TabsTrigger value="active">
            Đang hiệu lực
            <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-primary/10">{activeCount}</span>
          </TabsTrigger>
          <TabsTrigger value="inactive">
            Đã hết hạn
            <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-primary/10">{inactiveCount}</span>
          </TabsTrigger>
        </TabsList>
      </Tabs>
                ...
                paginatedContracts.map((contract: Contract) => {
                  ...
                  return (
                    <TableRow key={`contract-${contract.id}-${contract.employee.id}`}>
                      ...
                      <TableCell>
                        <span className="text-sm">{getContractTypeName(contract.contractType)}</span>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1 justify-center">
                          {isAdmin && status === 'ACTIVE' && (
                            <Button variant="ghost" size="sm" onClick={() => handleOpenFormModal(contract)} title="Chỉnh sửa">
                              <Edit className="h-4 w-4" />
                            </Button>
                          )}
                          <Button variant="ghost" size="sm" onClick={() => handleOpenDetailModal(contract.id)} title="Xem chi tiết">
                            <Eye className="h-4 w-4" />
                          </Button>
                          {isAdmin && (
                            <Button variant="ghost" size="sm" onClick={() => handleDelete(contract.id)} title="Xóa" className="text-red-600 hover:text-red-700">
