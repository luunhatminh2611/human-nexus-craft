...
export default function KPIManagement() {
  ...
  // Xác định nhân viên có thể giao KPI
  const getAssignableEmployees = () => {
    const currentUser = mockData.employees.find(e => e.id === employeeId);
    
    if (role === 'Director') {
      // Giám đốc giao cho Phó giám đốc
      return mockData.employees.filter(e => e.position.includes('Phó giám đốc'));
    } else if (role === 'ViceDirector') {
      // Phó giám đốc giao cho Trưởng phòng
      return mockData.employees.filter(e => e.position.includes('Trưởng phòng'));
    } else if (role === 'DepartmentHead') {
      // Trưởng phòng giao cho Phó phòng trong phòng của mình
      return mockData.employees.filter(e => 
        e.position.includes('Phó phòng') && e.departmentId === currentUser?.departmentId
      );
    } else if (role === 'DeputyHead') {
      // Phó phòng giao cho nhân viên trong phòng của mình
      return mockData.employees.filter(e => 
        e.departmentId === currentUser?.departmentId && 
        !e.position.includes('Trưởng phòng') &&
        !e.position.includes('Phó phòng')
      );
    }
    return [];
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Giao và theo dõi KPI cho cấp dưới</p>
          </div>
